...
a_req_stays_req_on_cache_hit: assert property (((current_state == S_REQ) && pte_cache_hit && (count_q < $unsigned(LEVELS-1)) |=> current_state == S_REQ))
else begin
    $display("PTW in REQ state with a cache hit should have stayed in REQ state for the next level walk.");
end
...
a_g_stage_error_implies_g_stage: assert property ((ptw_tlb_comm.resp.g_stage_error |-> is_g_stage_q))
else begin
    $display("A G-stage error was reported when not in a G-stage walk.");
end
...
a_vs_to_g_saves_context: assert property ((current_state == S_VS_TO_G |-> (vs_count_reg_d == count_q) && (vs_pte_addr_reg_d == aux_pte_addr[SIZE_VADDR:0])))
else begin
    $display("When transitioning from VS-stage to G-stage, the VS context (level and address) was not saved correctly.");
end

a_g_to_vs_restores_context: assert property ((current_state == S_G_TO_VS |-> count_d == vs_count_reg_q))
else begin
    $display("When transitioning from G-stage back to VS-stage, the VS level was not restored correctly.");
end

a_vs_to_g_resets_count: assert property ((current_state == S_VS_TO_G |-> count_d == 0))
else begin
    $display("When transitioning from VS-stage to G-stage, the walk counter was not reset to zero.");
end

a_g_to_vs_clears_g_stage_flag: assert property ((current_state == S_G_TO_VS |-> is_g_stage_d == 0))
else begin
    $display("When transitioning from G-stage to VS-stage, the is_g_stage flag was not cleared.");
end
...
a_resp_implies_ready_soon: assert property ((ptw_itlb_comm_o.resp.valid || ptw_dtlb_comm_o.resp.valid |=> ptw_itlb_comm_o.ptw_ready && ptw_dtlb_comm_o.ptw_ready))
else begin
    $display("PTW did not become ready in the cycle after sending a response.");
end

a_dmem_req_stable_when_not_ready: assert property ((ptw_dmem_comm_o.req.valid && !dmem_ptw_comm_i.dmem_ready |=> $stable(ptw_dmem_comm_o.req)))
else begin
    $display("DMEM request was changed while dmem_ready was low.");
end
...
a_dmem_nack_retry_addr_stable: assert property ((dmem_ptw_comm_i.resp.nack && $past(ptw_dmem_comm_o.req.valid) |=> ##1 ptw_dmem_comm_o.req.addr == $past(ptw_dmem_comm_o.req.addr, 2)))
else begin
    $display("PTW retried a DMEM request after a NACK but with a different address.");
end

a_no_pmu_hit_after_flush: assert property ((pmu_ptw_hit_o |-> !$past(csr_ptw_comm_i.flush)))
else begin
    $display("A PMU hit was reported in the cycle immediately following a global flush.");
end

a_wait_to_done_on_leaf: assert property (((current_state == S_WAIT) && dmem_ptw_comm_i.resp.valid && !invalid_pte && is_pte_leaf && !(is_virtualized_env & is_g_stage_q & ~last_g_stage_q) && !(is_virtualized_env & r_req.en_g & is_vs_stage) |=> current_state == S_DONE))
else begin
    $display("PTW received a valid leaf PTE but did not transition to DONE state.");
end

a_wait_to_error_on_invalid_pte: assert property (((current_state == S_WAIT) && dmem_ptw_comm_i.resp.valid && invalid_pte |=> current_state == S_ERROR))
else begin
    $display("PTW received an invalid PTE from memory but did not transition to the ERROR state.");
end

a_count_increments_on_walk: assert property (((current_state == S_WAIT) && dmem_ptw_comm_i.resp.valid && !invalid_pte && is_pte_table && (count_q < $unsigned(LEVELS-1)) |=> count_q == $past(count_q) + 1))
else begin
    $display("Page walk level counter did not increment after fetching a table PTE.");
end
