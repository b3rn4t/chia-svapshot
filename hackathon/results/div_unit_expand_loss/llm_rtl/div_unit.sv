...
logic [63:0] div_d_eff;
logic [63:0] div_r_eff;
logic early_out_d;
...
//--------------------------------------------------------------------------------------------------
    ...
    assign div_d_eff = instruction_i.instr.op_32 ? {32'b0, dividend_d[31:0]} : dividend_d;
    assign div_r_eff = instruction_i.instr.op_32 ? {32'b0, divisor_d[31:0]} : divisor_d;
    assign early_out_d = (div_d_eff < div_r_eff);
...
//--------------------------------------------------------------------------------------------------
    ...
    always_ff @(posedge clk_i, negedge rstn_i) begin
        if (~rstn_i) begin
            ...
            remanent_q[div_unit_sel_i]           <= early_out_d ? div_d_eff : 'h0;
            dividend_quotient_q[div_unit_sel_i]  <= early_out_d ? 'h0 : ((instruction_i.instr.op_32) ? {dividend_d[31:0],32'b0} : dividend_d);
            divisor_q[div_unit_sel_i]            <= (instruction_i.instr.op_32) ? {32'b0, divisor_d[31:0]} : divisor_d;
            
            cycles_counter[div_unit_sel_i]        <= early_out_d ? 6'd1 : ((instruction_i.instr.op_32) ? 6'd17 : 6'd33);
