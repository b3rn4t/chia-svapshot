...
rm -rf "$work"
mkdir -p "$work/ft_ptw/sva" "$work/vcf_projs/ptw"

if [[ "$rtl" == original ]]; then
  rtl_dir="$ROOT/original_rtl"
  prop="$ROOT/sva/original/ptw_prop.sv"
else
  rtl_dir="$ROOT/llm_rtl"
  prop="$ROOT/sva/new/ptw_prop.sv"
fi

cp -a "$rtl_dir"/. "$work/"
cp "$prop" "$work/ft_ptw/sva/ptw_prop.sv"
cp "$ROOT/bind/ptw_bind.svh" "$work/ft_ptw/sva/ptw_bind.svh"

python3 - <<PY
from pathlib import Path
work = Path("$work")
names = ["riscv_pkg.sv", "mmu_pkg.sv", "pseudoLRU.sv", "ptw_arb.sv", "ptw.sv"]
abs_files = []
for name in names:
    p = work / name
    if p.is_file():
        abs_files.append(str(p.resolve()))
prop = (work / "ft_ptw/sva/ptw_prop.sv").resolve()
bind = (work / "ft_ptw/sva/ptw_bind.svh").resolve()
vc = work / "ft_ptw/files_vcf.vc"
lines = [
    "+verilog2001ext+.v",
    "+verilog2001ext+.vh",
    "+systemverilogext+.sv",
...
vc.write_text("\n".join(lines) + "\n")
tcl = work / "ft_ptw/FPV_vcf.tcl"
tcl.write_text("""# Replay FPV for ptw
set top ptw
set_fml_appmode FPV
...
set report_dir "vcf_projs/ptw/reports"
...
LOG="$work/vcf_projs/ptw/vcf.log"
mkdir -p "$(dirname "$LOG")"
...
run_vcf() {
  if [[ -f /.dockerenv ]]; then
    ...
    (cd "$work" && vcf -session "vcf_projs/ptw/vcst_rtdb" -no_restore -f "ft_ptw/FPV_vcf.tcl" -batch)
    return
  fi
  ...
  docker exec \
    ...
    "$CONTAINER" \
    "$VCF_HOME/bin/vcf" \
    -session "vcf_projs/ptw/vcst_rtdb" \
    -no_restore \
    -f "ft_ptw/FPV_vcf.tcl" \
    -batch
}
