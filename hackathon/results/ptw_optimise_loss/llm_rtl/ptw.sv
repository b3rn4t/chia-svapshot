...
assign vs_req_blocked = is_virtualized_env & is_vs_stage & ~allow_vs_req_q;
...
logic [PPN_SIZE-3:0] mux_ppn_high;
logic [PAGE_LVL_BITS+1:0] mux_vpn;

always_comb begin
    if (is_virtualized_env & is_g_stage_q) begin
        if ((count_q == 0) && (current_state != S_DONE) && (current_state != S_G_TO_VS)) begin
            mux_ppn_high = csr_ptw_comm_i.hgatp[PPN_SIZE-1:2];
            mux_vpn = g_vpn0_req;
        end else begin
            mux_ppn_high = r_pte.ppn[PPN_SIZE-1:2];
            mux_vpn = {r_pte.ppn[1:0], g_vpn_idx};
        end
    end else begin
        mux_ppn_high = r_pte.ppn[PPN_SIZE-1:2];
        mux_vpn = {r_pte.ppn[1:0], vpn_idx};
    end
end

logic [63:0] unified_pte_addr;
assign unified_pte_addr = {{(64-(PPN_SIZE+PAGE_LVL_BITS+$clog2(riscv_pkg::XLEN/8))){1'b0}}, 
                           mux_ppn_high, 
                           mux_vpn, 
                           {{($clog2(riscv_pkg::XLEN/8))}{1'b0}}};

assign pte_addr = unified_pte_addr[SIZE_VADDR:0];
...
always_ff @(posedge clk_i, negedge rstn_i) begin
    if (!rstn_i) begin
        ...
        if ((current_state == S_WAIT) && dmem_ptw_comm_i.resp.valid) begin
            r_pte <= pte;
        end else if ((current_state == S_REQ) && pte_cache_hit && (count_q < $unsigned(LEVELS-1)) && !vs_req_blocked) begin
...
always_ff @(posedge clk_i, negedge rstn_i) begin
    if (!rstn_i) begin
        ...
        else if (pte_cache_hit && ((current_state == S_REQ) && !vs_req_blocked)) begin
            access_hit <= 1'b1;
        end
...
always_comb begin
    for (int i = 0; i < PTW_CACHE_SIZE; i++) begin
        hit_vector[i] = ((ptecache_entry[i].tags == pte_addr) &&
                         ...
                         (ptecache_entry[i].vs_enable == (is_virtualized_env & is_vs_stage)) && 
                         !vs_req_blocked &&
...
always_comb begin
    hit_idx = '0;
    pte_cache_data = '0;
    for (int i = PTW_CACHE_SIZE-1; i >= 0; i--) begin
        if (hit_vector[i]) begin
            hit_idx = trunc_ptw_cache_size($unsigned(i));
...
assign ptw_tlb_comm.resp.g_stage_error = resp_err & is_g_stage_q;
assign ptw_tlb_comm.resp.level = count_q;
assign ptw_tlb_comm.resp.pte.ppn = is_virtualized_env? g_resp_ppn : resp_ppn;
...
assign ptw_tlb_comm.resp.virt_level = virt_level_q;
assign ptw_tlb_comm.resp.guest_pte = guest_pte_q;
...
always_comb begin
    ...
    case (current_state)
        ...
        S_REQ : begin
            if (vs_req_blocked & r_req.en_g) begin
                ptw_dmem_comm_o.req.valid = 1'b0;
                pmu_ptw_hit_o = 1'b0;
                count_d = count_q;
                next_state = S_VS_TO_G;
            end else if (pte_cache_hit && (count_q < $unsigned(LEVELS-1))) begin
                ptw_dmem_comm_o.req.valid = 1'b0;
                pmu_ptw_hit_o = 1'b1;
                count_d = count[1:0];
                allow_vs_req_d = (is_virtualized_env & is_g_stage_q);
                next_state = S_REQ;
            end else begin
